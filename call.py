#!/usr/bin/env python3
"""Place an outbound call that Claude conducts on your behalf.

    ./call.py --to +14155551234 --goal "Book a table for 2 on Friday at 7pm under Jordan"
    ./call.py --to +14155551234 --goal "..." --wait

Without --wait it returns the call SID immediately; the outcome is reported to
Discord when the call ends. With --wait it blocks and prints the summary.
"""
import argparse
import json
import re
...
def main() -> int:
    ...
    ap.add_argument("--goal", required=True, help="What Claude should accomplish on the call")
